...
import 'package:chat/Utils/custom_url_launcher.dart';
import 'package:chat/Utils/setStatusBarColor.dart';
import 'package:chat/widgets/AllinOneCameraGalleryImageVideoPicker/AllinOneCameraGalleryImageVideoPicker.dart';
import 'package:chat/widgets/CameraGalleryImagePicker/camera_image_gallery_picker.dart';
import 'package:chat/widgets/CameraGalleryImagePicker/multiMediaPicker.dart';
import 'package:chat/widgets/DownloadManager/download_all_file_type.dart';
import 'package:chat/widgets/VideoEditor/video_editor.dart';
import 'package:path/path.dart' as p;
import 'package:collection/collection.dart';
import 'package:emoji_picker_flutter/emoji_picker_flutter.dart';
import 'package:chat/Configs/app_constants.dart';
import 'package:chat/Configs/optional_constants.dart';
import 'package:chat/Screens/auth_screens/login.dart';
import 'package:chat/Screens/chat_screen/utils/aes_encryption.dart';
import 'package:chat/Screens/chat_screen/utils/uploadMediaWithProgress.dart';
import 'package:chat/Screens/contact_screens/SelectContactsToForward.dart';
import 'package:chat/Screens/security_screens/security.dart';
import 'package:chat/Services/Admob/admob.dart';
import 'package:chat/Utils/emoji_detect.dart';
import 'package:chat/Utils/mime_type.dart';
import 'package:chat/main.dart';
import 'package:chat/widgets/CountryPicker/CountryCode.dart';
import 'package:chat/Configs/Dbkeys.dart';
import 'package:chat/Configs/Dbpaths.dart';
import 'package:chat/Screens/chat_screen/utils/deleteChatMedia.dart';
import 'package:chat/Screens/privacypolicy&TnC/PdfViewFromCachedUrl.dart';
import 'package:chat/Services/Providers/Observer.dart';
import 'package:chat/widgets/MultiDocumentPicker/multiDocumentPicker.dart';
import 'package:chat/widgets/MyElevatedButton/MyElevatedButton.dart';
import 'package:chat/widgets/SoundPlayer/SoundPlayerPro.dart';
...
import 'package:chat/Screens/call_history/callhistory.dart';
import 'package:chat/Screens/chat_screen/utils/audioPlayback.dart';
import 'package:chat/Screens/chat_screen/utils/message.dart';
import 'package:chat/Screens/contact_screens/ContactsSelect.dart';
import 'package:chat/Models/DataModel.dart';
import 'package:chat/Screens/chat_screen/utils/photo_view.dart';
import 'package:chat/Screens/profile_settings/profile_view.dart';
import 'package:chat/Services/Providers/seen_provider.dart';
import 'package:chat/Services/Providers/seen_state.dart';
import 'package:chat/Screens/calling_screen/pickup_layout.dart';
import 'package:chat/Utils/call_utilities.dart';
import 'package:chat/Utils/permissions.dart';
import 'package:chat/Utils/chat_controller.dart';
import 'package:chat/Utils/crc.dart';
import 'package:chat/Utils/open_settings.dart';
import 'package:chat/Utils/save.dart';
import 'package:chat/Utils/utils.dart';
import 'package:chat/widgets/AudioRecorder/Audiorecord.dart';
import 'package:chat/widgets/ImagePicker/image_picker.dart';
import 'package:chat/widgets/VideoPicker/VideoPreview.dart';
import 'package:chat/Screens/chat_screen/Widget/bubble.dart';
...
import 'package:chat/Models/E2EE/e2ee.dart' as e2ee;
import 'package:chat/Utils/unawaited.dart';
...
class _ChatScreenState extends State<ChatScreen> with WidgetsBindingObserver {
  ...
  String decryptWithCRC(String input) {
    ...
    // chat.toast(getTranslated(this.context, 'msgnotload'));
    return '';
  }
    ...
    if (content.trim() != '') {
      ...
      try {
        ...
        if (encrypted is String) {
          ...
          chat.toast('Nothing to encrypt');
        }
      } on Exception catch (_) {
        // print('Exception caught!');
        chat.toast("Exception: $_");
      }
    }
  }
  ...
  contextMenuNew(contextForDialog, Map<String, dynamic> mssgDoc, bool isTemp,
    ...
    if ((mssgDoc[Dbkeys.from] == currentUserNo &&
      ...
      tiles.add(ListTile(
          ...
          onTap: () async {
            chat.toast(getTranslated(contextForDialog, 'deleting'));
            await FirebaseFirestore.instance
                .collection(DbPaths.collectionmessages)
                .doc(chatId)
                .collection(chatId!)
                ...
                .then((chatDoc) async {
              if (!chatDoc.exists) {
                chat.toast('Please reload this screen !');
                  ...
                  if ((mssgDoc.containsKey(Dbkeys.isbroadcast) == true
                    ...
                    await deleteMsgMedia(realtimeDoc, chatId!)
                      ...
                      if (isDeleted == false || isDeleted == null) {
                        chat.toast('Could not delete. Please try again!');
                      } else {
                        await FirebaseFirestore.instance
                            .collection(DbPaths.collectionmessages)
                            .doc(chatId)
    ...
    if (index >= list.length) {
      ...
      if (list[index].containsKey(Dbkeys.groupNAME)) {
        ...
        try {
          ...
          chat.toast('Failed to Forward message. Error:$e');
        }
      }
    }
  }
                          ...
                          Container(
                              child: Row(
                                ...
                                Icon(
                                  ...
                                  size: 12,
                                  color: chatGrey.withOpacity(0.5),
                                ),
                                SizedBox(
                                  width: 5,
                                ),
                                Text(getTranslated(this.context, 'forwarded'),
                                    maxLines: 1,
                                    style: TextStyle(
                                        color: chatGrey.withOpacity(0.7),
          ...
          ElevatedButton(
              onPressed: () async {
                ...
                if (isUser == null || isUser == false) {
                  chat.toast(getTranslated(this.context, 'usernotjoined') +
                      ' $Appname');
                }
              },
  ...
  replyAttachedWidget(BuildContext context, var doc) {
    return Flexible(
      child: Container(
          ...
          decoration: BoxDecoration(
              color: chatWhite.withOpacity(0.55),
          ...
          child: Stack(
            children: [
              Container(
                  ...
                  decoration: BoxDecoration(
                      color: chatGrey.withOpacity(0.1),
  ...
  //                 ? chatBlack.withOpacity(0.0)
  //                 : chatWhite.withOpacity(0.0),
  //           )
  //         : Container(),
  //   );
  // }
    ...
    return Positioned(
      child: isgeneratingSomethingLoader
          ? Container(
              ...
              color: DESIGN_TYPE == Themetype.whatsapp
                  ? chatBlack.withOpacity(0.6)
                  : chatWhite.withOpacity(0.6),
            )
          : Container(),
    );
  }
    ...
    if (chatStatus == ChatStatus.requested.index) {
      return AlertDialog(
        ...
        title: Text(
          ...
          style: TextStyle(color: chatBlack),
        ),
        actions: <Widget>[
          // ignore: deprecated_member_use
          ElevatedButton(
          ...
          ElevatedButton(
              child: Text(getTranslated(this.context, 'acpt'),
                  style: TextStyle(color: chatgreen)),
              onPressed: () {
                    ...
                    child: IconButton(
                      icon: new Icon(
                        textEditingController.text.length == 0
                            ? Icons.mic
                            : Icons.send,
                        color: chatWhite.withOpacity(0.99),
                      ),
                      onPressed: isMessageLoading == true
                          ? null
                          : observer.ismediamessagingallowed == true
                              ? textEditingController.text.length == 0
                                  ...
                                  : observer.istextmessagingallowed == false
                                      ? () {
                                          chat.showRationale(getTranslated(
                                              this.context,
                                              'textmssgnotallowed'));
                                        }
                                      : chatStatus == ChatStatus.blocked.index
                  ...
                  child: SizedBox(
                    ...
                    child: EmojiPicker(
                        ...
                        config: Config(
                            ...
                            indicatorColor: chatgreen,
                            iconColor: Colors.grey,
                            iconColorSelected: chatgreen,
                            // progressIndicatorColor: Colors.blue,
                            backspaceColor: chatgreen,
                            showRecentsTab: true,
                            recentsLimit: 28,
  ...
  //                     textCapitalization: TextCapitalization.sentences,
  //                     maxLines: null,
  //                     style: TextStyle(fontSize: 18.0, color: chatBlack),
  //                     controller: textEditingController,
  //                     decoration: InputDecoration(
  //                       enabledBorder: OutlineInputBorder(
  //                         // width: 0.0 produces a thin "hairline" border
...
//-- GROUP BY DATE ---
  ...
  Widget build(BuildContext context) {
    ...
    return PickupLayout(
      prefs: widget.prefs,
      scaffold: chat.getNTPWrappedWidget(WillPopScope(
